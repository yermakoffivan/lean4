...
This module is provides a system for constructing efficient for efficiently
elaborating and evaluating configuration syntaxes.
...
macros that construct configuration elaborators from lower-level components, which
metaprogram authors directly interface with instead.

These commands handle the common case where
...
3. all fields have types that are composed from `Option`, `List`, `Array`, `String`,
  `DataValue`, and inductive types in `Type` with no parameters or indices, whose
  fields are similarly composed.

The macros synthesize a self-contained configuration elaboration procedure, analyzing
the `EvalTerm` and `EvalExpr` instances that are currently available or can be automatically
derived. These are the components of the system:
...
- Configuration item interpretation is through `ConfigEval.foldConfigM`, which is a
  procedure with a lax specification for what counts as a configuration item, calling the
  provided function on each recognized configuration item. The idea is:
...
- Configuration evaluation then uses `EvalConfigItem.set` on each item produced by the
  ...
  The `def_eval_config_item` command can be used to generate this structure. It
...
the additional instances be a side effect of the macros. The instances can be globally
added by manually using the `ensure_*` commands.
...
that their use ought to be avoided in the prelude.
