...
auto-generated by the implementation. Stripping a user-set Origin would break ntentional
...
private def scrubHeaders (headers : Headers) (isCrossOrigin methodChangedToSafe : Bool) : Headers :=
  let afterOrigin :=
    ...
    if isCrossOrigin then
      connectionHeaders ++ clientProxyHeaders ++ originHeaders ++ validatingHeaders
    else
      #[]
