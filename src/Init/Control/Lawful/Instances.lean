...
@[simp, grind =, defeq] theorem monadMap_refl {m : Type _ → Type _} {α} (f : ∀ {α}, m α → m α) :
    monadMap @f = @f α := rfl
...
@[simp, defeq] theorem stM_eq [Monad m] : stM m (ExceptT ε m) α = Except ε α := rfl

@[simp, grind =, backward_defeq] theorem run_mk (x : m (Except ε α)) : run (mk x : ExceptT ε m α) = x := rfl

@[simp, grind =, backward_defeq] theorem run_pure [Monad m] (x : α) : run (pure x : ExceptT ε m α) = pure (Except.ok x) := rfl

@[simp, grind =, backward_defeq] theorem run_lift  [Monad.{u, v} m] (x : m α) : run (ExceptT.lift x : ExceptT ε m α) = (Except.ok <$> x : m (Except ε α)) := rfl

@[simp, grind =, backward_defeq] theorem run_throw [Monad m] : run (throw e : ExceptT ε m β) = pure (Except.error e) := rfl
...
@[simp, grind =, backward_defeq] theorem run_monadMap [MonadFunctorT n m] (f : {β : Type u} → n β → n β) (x : ExceptT ε m α)
...
@[simp, backward_defeq] theorem run_restoreM [Monad m] (x : stM m (ExceptT ε m) α) :
    ExceptT.run (restoreM x) = pure x := rfl

@[simp, backward_defeq] theorem run_liftWith [Monad m] (f : ({β : Type u} → ExceptT ε m β → m (stM m (ExceptT ε m) β)) → m α) :
...
@[simp, grind =, backward_defeq] theorem run_monadMap [MonadFunctorT n m] (f : {β : Type u} → n β → n β) (x : OptionT m α)
...
@[simp, backward_defeq] theorem run_restoreM [Monad m] (x : stM m (OptionT m) α) :
...
@[simp, grind =, backward_defeq] theorem run_mk (x : ρ → m α) (ctx : ρ) : run (.mk x : ReaderT ρ m α) ctx = x ctx :=
  rfl

@[simp, grind =, backward_defeq] theorem run_pure [Monad m] (a : α) (ctx : ρ) : (pure a : ReaderT ρ m α).run ctx = pure a := rfl

@[simp, grind =, backward_defeq] theorem run_bind [Monad m] (x : ReaderT ρ m α) (f : α → ReaderT ρ m β) (ctx : ρ)
    : (x >>= f).run ctx = x.run ctx >>= λ a => (f a).run ctx := rfl

@[simp, grind =, backward_defeq] theorem run_mapConst [Monad m] (a : α) (x : ReaderT ρ m β) (ctx : ρ)
    : (Functor.mapConst a x).run ctx = Functor.mapConst a (x.run ctx) := rfl

@[simp, grind =, backward_defeq] theorem run_map [Monad m] (f : α → β) (x : ReaderT ρ m α) (ctx : ρ)
    : (f <$> x).run ctx = f <$> x.run ctx := rfl

@[simp, grind =, backward_defeq] theorem run_monadLift [MonadLiftT n m] (x : n α) (ctx : ρ)
    : (monadLift x : ReaderT ρ m α).run ctx = (monadLift x : m α) := rfl

@[simp, grind =, backward_defeq] theorem run_monadMap [MonadFunctorT n m] (f : {β : Type u} → n β → n β) (x : ReaderT ρ m α) (ctx : ρ)
    : (monadMap @f x : ReaderT ρ m α).run ctx = monadMap @f (x.run ctx) := rfl

@[simp, grind =, backward_defeq] theorem run_read [Monad m] (ctx : ρ) : (ReaderT.read : ReaderT ρ m ρ).run ctx = pure ctx := rfl

@[simp, backward_defeq] theorem run_seq {α β : Type u} [Monad m] (f : ReaderT ρ m (α → β)) (x : ReaderT ρ m α) (ctx : ρ)
    : (f <*> x).run ctx = (f.run ctx <*> x.run ctx) := rfl

@[simp, backward_defeq] theorem run_seqRight [Monad m] (x : ReaderT ρ m α) (y : ReaderT ρ m β) (ctx : ρ)
    : (x *> y).run ctx = (x.run ctx *> y.run ctx) := rfl

@[simp, backward_defeq] theorem run_seqLeft [Monad m] (x : ReaderT ρ m α) (y : ReaderT ρ m β) (ctx : ρ)
    : (x <* y).run ctx = (x.run ctx <* y.run ctx) := rfl
...
@[simp, backward_defeq] theorem run_restoreM [Monad m] (x : stM m (ReaderT ρ m) α) (ctx : ρ) :
    ReaderT.run (restoreM x) ctx = pure x := rfl

@[simp, backward_defeq] theorem run_liftWith [Monad m] (f : ({β : Type u} → ReaderT ρ m β → m (stM m (ReaderT ρ m) β)) → m α) (ctx : ρ) :
...
@[simp, grind =, backward_defeq] theorem run_mk [Monad m] (x : σ → m (α × σ)) (s : σ) : run (.mk x) s = x s :=
  rfl

@[simp, grind =, backward_defeq] theorem run'_eq [Monad m] (x : StateT σ m α) (s : σ) : run' x s = (·.1) <$> run x s :=
  rfl

@[simp, grind =, backward_defeq] theorem run_pure [Monad m] (a : α) (s : σ) : (pure a : StateT σ m α).run s = pure (a, s) := rfl

@[simp, grind =, backward_defeq] theorem run_bind [Monad m] (x : StateT σ m α) (f : α → StateT σ m β) (s : σ)
...
@[simp, grind =, backward_defeq] theorem run_get [Monad m] (s : σ)    : (get : StateT σ m σ).run s = pure (s, s) := rfl

@[simp, grind =, backward_defeq] theorem run_set [Monad m] (s s' : σ) : (set s' : StateT σ m PUnit).run s = pure (⟨⟩, s') := rfl

@[simp, grind =, backward_defeq] theorem run_modify [Monad m] (f : σ → σ) (s : σ) : (modify f : StateT σ m PUnit).run s = pure (⟨⟩, f s) := rfl

@[simp, grind =] theorem run_modifyGet [Monad m] (f : σ → α × σ) (s : σ) : (modifyGet f : StateT σ m α).run s = pure ((f s).1, (f s).2) := by
  rfl

@[simp, grind =, backward_defeq] theorem run_lift {α σ : Type u} [Monad m] (x : m α) (s : σ) : (StateT.lift x : StateT σ m α).run s = x >>= fun a => pure (a, s) := rfl
...
@[simp, grind =, backward_defeq] theorem run_monadLift {α σ : Type u} [Monad m] [MonadLiftT n m] (x : n α) (s : σ) : (monadLift x : StateT σ m α).run s = (monadLift x : m α) >>= fun a => pure (a, s) := rfl

@[simp, grind =, backward_defeq] theorem run_monadMap [MonadFunctorT n m] (f : {β : Type u} → n β → n β) (x : StateT σ m α) (s : σ) :
...
@[simp, grind =, backward_defeq] theorem run_pure (a : α) (s : σ) :
    EStateM.run (pure a : EStateM ε σ α) s = .ok a s := rfl

@[simp, grind =, backward_defeq] theorem run_get (s : σ) :
    EStateM.run (get : EStateM ε σ σ) s = .ok s s := rfl

@[simp, grind =, backward_defeq] theorem run_set (s₁ s₂ : σ) :
    EStateM.run (set s₁ : EStateM ε σ PUnit) s₂ = .ok .unit s₁ := rfl

@[simp, grind =, backward_defeq] theorem run_modify (f : σ → σ) (s : σ) :
    EStateM.run (modify f : EStateM ε σ PUnit) s = .ok .unit (f s) := rfl

@[simp, grind =, backward_defeq] theorem run_modifyGet (f : σ → α × σ) (s : σ) :
    EStateM.run (modifyGet f : EStateM ε σ α) s = .ok (f s).1 (f s).2 := rfl

@[simp, grind =, backward_defeq] theorem run_throw (e : ε) (s : σ):
    EStateM.run (throw e : EStateM ε σ PUnit) s = .error e s := rfl

@[simp, grind =, backward_defeq] theorem run_bind (x : EStateM ε σ α) (f : α → EStateM ε σ β)
