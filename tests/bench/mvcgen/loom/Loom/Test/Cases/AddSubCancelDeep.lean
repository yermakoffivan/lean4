...
provable by `lmvcgen` in Loom (see the divergence notes in `Loom/Test/AddSubCancelDeep.lean`),
...
def runTests := runBenchUsingTactic
    ``Goal [``loop, ``step]
    `(tactic| (intro post epost s₁ s₂; lmvcgen simplifying_assumptions with grind))
    `(tactic| fail)
