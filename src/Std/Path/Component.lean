...
A parsed segment of a file system path. Paths are stored as arrays of components,
enabling pure manipulation without platform-specific string scanning at each operation.
-/
...
Paths are stored as `Array Component`, so all structural operations (parent, join,
normalize) work directly on this array without re-scanning strings.
-/
