...
@[backward_defeq, simp, grind =] theorem entails_nil {P Q : SPred []} : entails P Q = (P.down → Q.down) := rfl
...
@[backward_defeq, simp, grind =] theorem bientails_nil {P Q : SPred []} : bientails P Q = (P.down ↔ Q.down) := rfl
@[backward_defeq]
...
@[backward_defeq, simp, grind =] theorem and_nil {P Q : SPred []} : and P Q = ⟨P.down ∧ Q.down⟩ := rfl
@[backward_defeq, simp, grind =] theorem and_cons {P Q : SPred (σ::σs)} : and P Q s = and (P s) (Q s) := rfl
...
@[backward_defeq, simp, grind =] theorem or_nil {P Q : SPred []} : or P Q = ⟨P.down ∨ Q.down⟩ := rfl
@[backward_defeq, simp, grind =] theorem or_cons {P Q : SPred (σ::σs)} : or P Q s = or (P s) (Q s) := rfl

/-- Negation in `SPred`: states that do not satisfy `P` satisfy `spred(¬ P)`. -/
def not {σs : List (Type u)} (P : SPred σs) : SPred σs := match σs with
  | [] => ⟨¬ P.down⟩
  | σ :: _ => fun (s : σ) => not (P s)
@[backward_defeq, simp, grind =] theorem not_nil {P : SPred []} : not P = ⟨¬ P.down⟩ := rfl
@[backward_defeq, simp, grind =] theorem not_cons {P : SPred (σ::σs)} : not P s = not (P s) := rfl
...
@[backward_defeq, simp, grind =] theorem imp_nil {P Q : SPred []} : imp P Q = ⟨P.down → Q.down⟩ := rfl
@[backward_defeq, simp, grind =] theorem imp_cons {P Q : SPred (σ::σs)} : imp P Q s = imp (P s) (Q s) := rfl
...
@[backward_defeq, simp, grind =] theorem iff_nil {P Q : SPred []} : iff P Q = ⟨P.down ↔ Q.down⟩ := rfl
@[backward_defeq, simp, grind =] theorem iff_cons {P Q : SPred (σ::σs)} : iff P Q s = iff (P s) (Q s) := rfl
...
@[backward_defeq, simp, grind =] theorem exists_nil {α} {P : α → SPred []} : «exists» P = ⟨∃ a, (P a).down⟩ := rfl
@[backward_defeq, simp] theorem exists_cons {α} {P : α → SPred (σ::σs)} : «exists» P s = «exists» (fun a => P a s) := rfl
...
@[backward_defeq, simp, grind =] theorem forall_nil {α} {P : α → SPred []} : «forall» P = ⟨∀ a, (P a).down⟩ := rfl
@[backward_defeq, simp, grind =] theorem forall_cons {α} {P : α → SPred (σ::σs)} : «forall» P s = «forall» (fun a => P a s) := rfl
...
@[backward_defeq, simp, grind =] theorem conjunction_nil : conjunction ([] : List (SPred σs)) = pure True := rfl
@[backward_defeq, simp, grind =] theorem conjunction_cons {P : SPred σs} {env : List (SPred σs)} : conjunction (P::env) = P.and (conjunction env) := rfl
