...
theorem Spec.MonadState_set_StateT
    ...
    Triple (fun _ => post ⟨⟩ s) (MonadStateOf.set s : StateT σ m PUnit) post epost := by
...
theorem Spec.MonadStateOf_get_StateT_lift
    ...
    [Monad m] [MonadStateOf σ m] [Assertion Pred] [Assertion EPred] [WPMonad m Pred EPred]
    {σ' : Type u} (post : σ → σ' → Pred) (epost : EPred) :
    Triple (wp (MonadLift.monadLift (n := StateT σ' m) (MonadStateOf.get : m σ)) post epost)
...
theorem Spec.MonadStateOf_set_ExceptT
    ...
    Triple (wp (MonadLift.monadLift (n := ExceptT ε m) (MonadStateOf.set s : m PUnit)) post epost)
      (MonadStateOf.set s : ExceptT ε m PUnit) post epost := by
  apply Triple.iff.mpr
...
theorem Spec.MonadStateOf_set_ReaderT
    ...
    Triple (wp (MonadLift.monadLift (n := ReaderT ρ m) (MonadStateOf.set s : m PUnit)) post epost)
      (MonadStateOf.set s : ReaderT ρ m PUnit) post epost := by
  exact Triple.iff.mpr (by
    intro _
    rfl)
...
theorem Spec.MonadStateOf_set_StateT_lift
    ...
    [Monad m] [MonadStateOf σ m] [Assertion Pred] [Assertion EPred] [WPMonad m Pred EPred]
    {σ' : Type u} {s : σ} (post : PUnit → σ' → Pred) (epost : EPred) :
    Triple (wp (MonadLift.monadLift (n := StateT σ' m) (MonadStateOf.set s : m PUnit)) post epost)
      (MonadStateOf.set s : StateT σ' m PUnit) post epost := by
  exact Triple.iff.mpr (by
    intro _
    rfl)
