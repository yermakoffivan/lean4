...
@[simp, backward_defeq] theorem pmap_empty {P : α → Prop} (f : ∀ a, P a → β) : pmap f #[] (by simp) = #[] := rfl
...
@[simp, backward_defeq] theorem attach_empty : (#[] : Array α).attach = #[] := rfl

@[simp, backward_defeq] theorem attachWith_empty {P : α → Prop} (H : ∀ x ∈ #[], P x) : (#[] : Array α).attachWith P H = #[] := rfl
