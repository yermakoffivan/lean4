...
@[simp, backward_defeq] theorem map_id : Prod.map (@id α) (@id β) = id := rfl

@[simp, backward_defeq] theorem map_id' : Prod.map (fun a : α => a) (fun b : β => b) = fun x ↦ x := rfl
