...
private def formatWith (locale : Locale) (modifier : Modifier) (data : TypeFormat modifier) : String :=
  ...
  | .G format =>
    ...
    | .twoLetterShort => formatEraShort locale.symbols data
  | .y format =>
    let info := data.toInt
  ...
  | .Q format | .q format =>
    ...
    | .inr .twoLetterShort => formatQuarterShort locale.symbols data
  | .w format =>
    pad format.padding data.val
  | .W format =>
    pad format.padding data.val
...
private def parseWith (config : FormatConfig) : (mod : Modifier) → Parser (TypeFormat mod)
  | .G format =>
    ...
    | .twoLetterShort => parseEraShort config.locale.symbols
  | .y format =>
    match format with
  ...
  | .Q format | .q format =>
    ...
    | .inr .twoLetterShort => parseQuarterShort config.locale.symbols
