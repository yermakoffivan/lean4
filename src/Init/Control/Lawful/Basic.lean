...
@[simp, defeq] theorem monadLift_self {m : Type u → Type v} (x : m α) : monadLift x = x :=
  rfl
...
@[simp, grind =, defeq] theorem run_map (x : Id α) (f : α → β) : (f <$> x).run = f x.run := rfl
@[simp, grind =, defeq] theorem run_bind (x : Id α) (f : α → Id β) : (x >>= f).run = (f x.run).run := rfl
@[simp, grind =, defeq] theorem run_pure (a : α) : (pure a : Id α).run = a := rfl
@[simp, grind =, defeq] theorem pure_run (a : Id α) : pure a.run = a := rfl
@[simp, defeq] theorem run_seqRight (x : Id α) (y : Id β) : (x *> y).run = y.run := rfl
@[simp, defeq] theorem run_seqLeft (x : Id α) (y : Id β) : (x <* y).run = x.run := rfl
@[simp, defeq] theorem run_seq (f : Id (α → β)) (x : Id α) : (f <*> x).run = f.run x.run := rfl
