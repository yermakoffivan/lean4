...
@[simp, backward_defeq] theorem mapM'_nil [Monad m] {f : α → m β} : mapM' f [] = pure [] := rfl
@[simp, backward_defeq] theorem mapM'_cons [Monad m] {f : α → m β} :
...
@[simp, grind =, backward_defeq] theorem mapM_nil [Monad m] {f : α → m β} : [].mapM f = pure [] := rfl
...
@[simp, grind =, backward_defeq] theorem filterMapM_nil [Monad m] {f : α → m (Option β)} : [].filterMapM f = pure [] := rfl
...
@[simp, grind =, backward_defeq] theorem zipWithM_nil_left [Monad m] {f : α → β → m γ} : zipWithM f [] l = pure (f := m) [] := rfl
...
@[simp, grind =, backward_defeq] theorem flatMapM_nil [Monad m] {f : α → m (List β)} : [].flatMapM f = pure [] := rfl
