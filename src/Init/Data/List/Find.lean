...
@[local simp, backward_defeq] private theorem findIdx?_go_nil {p : α → Bool} {i : Nat} :
    findIdx?.go p [] i = none := rfl

@[local simp, backward_defeq] private theorem findIdx?_go_cons :
...
@[simp, grind =, backward_defeq] theorem findIdx?_nil : ([] : List α).findIdx? p = none := rfl
...
@[simp, grind =, backward_defeq] theorem findFinIdx?_nil {p : α → Bool} : findFinIdx? p [] = none := rfl
...
@[simp, grind =, backward_defeq] theorem finIdxOf?_nil [BEq α] : ([] : List α).finIdxOf? a = none := rfl
...
@[simp, grind =, backward_defeq] theorem idxOf?_nil [BEq α] : ([] : List α).idxOf? a = none := rfl
