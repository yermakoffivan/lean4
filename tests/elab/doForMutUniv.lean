...
do not get stuck on `max (?u+1) (?v+1) =?= ?u+1`. This is solved by the `Level.dec` fallback
in `isLevelDefEq`'s `succ` case, which decomposes the constraint into `max ?u ?v =?= ?u`
and then applies `solveSelfMax`.
-/
