...
-- NB: This would test whether the syntax `:= rfl` heuristics would trigger on `Tricky.rfl`.
-- But since the this now only triggers `[infer_defeq]` and not `[defeq]` there is nothing to see anymore.
