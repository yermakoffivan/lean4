...
@[simp, grind =, backward_defeq] theorem toList_pop {xs : Array α} : xs.pop.toList = xs.toList.dropLast := rfl

@[simp, defeq] theorem append_eq_append {xs ys : Array α} : xs.append ys = xs ++ ys := rfl
...
@[simp, defeq] theorem toList_empty : (#[] : Array α).toList = [] := rfl
...
@[simp, defeq] theorem appendList_eq_append {xs : Array α} {l : List α} : xs.appendList l = xs ++ l := rfl
