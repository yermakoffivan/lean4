...
macro (name := mleave) "mleave" : tactic =>
  `(tactic| (try first
              ...
              | simp only [
                $(mkIdent ``Lean.Order.CompleteLattice.ofProp_fun_apply):term,
                $(mkIdent ``Std.Internal.Do.CompleteLattice.entails_1):term,
                $(mkIdent ``Std.Internal.Do.CompleteLattice.entails_2):term,
                $(mkIdent ``Std.Internal.Do.CompleteLattice.entails_3):term,
                $(mkIdent ``Std.Internal.Do.CompleteLattice.entails_4):term,
                $(mkIdent ``Std.Internal.Do.CompleteLattice.entails_5):term,
                $(mkIdent ``Lean.Order.entails_prop_eq_imp):term,
                $(mkIdent ``Lean.Order.meet_fun_apply):term,
                $(mkIdent ``Lean.Order.meet_prop_eq_and):term,
                $(mkIdent ``Lean.Order.join_fun_apply):term,
                $(mkIdent ``Lean.Order.join_prop_eq_or):term,
                $(mkIdent ``Lean.Order.himp_fun_apply):term,
                $(mkIdent ``Lean.Order.himp_prop_eq_imp):term,
                $(mkIdent ``Lean.Order.iSup_fun_apply):term,
                $(mkIdent ``Lean.Order.iSup_prop_eq_exists):term,
                $(mkIdent ``Lean.Order.iInf_fun_apply):term,
                $(mkIdent ``Lean.Order.iInf_prop_eq_forall):term,
                $(mkIdent ``Lean.Order.top_fun_apply):term,
                $(mkIdent ``Lean.Order.bot_fun_apply):term,
                $(mkIdent ``Lean.Order.CompleteLattice.ofProp_true):term,
                $(mkIdent ``Lean.Order.CompleteLattice.ofProp_false):term,
                $(mkIdent ``Lean.Order.PartialOrder.rel_refl):term,
                $(mkIdent ``Lean.Order.le_top):term,
                $(mkIdent ``Lean.Order.bot_le):term,
                $(mkIdent ``List.Cursor.prefix_at):term,
                $(mkIdent ``List.Cursor.suffix_at):term,
                $(mkIdent ``List.Cursor.current_at):term,
                $(mkIdent ``List.Cursor.tail_at):term,
