...
/-!
Helpers for the exception-postcondition (`EPost`) decomposition strategy in `solve`.

`tryEPostVC` reduces an `EPost.cons.head` projection in the RHS of an entailment to the underlying
component. The concrete case (`epost⟨…⟩`) is handled by `mkEPostAtIndex`/`peelEPostTailChain`; the
`⊥` case is handled by `replaceEPostHeadBot?`, which rewrites `pre ⊑ ⊥.head x₁ … xₙ` to `pre ⊑ ⊥`.
-/

/-- Get the `index`-th component from an `EPost` target. -/
def mkEPostAtIndex (target : Expr) (index : Nat) : SymM (Option Expr) := do
  let mut curr := target
  for _ in [:index] do
...
partial def peelEPostTailChain (curr : Expr) (idx : Nat := 0) : Expr × Nat :=
...
`bot_fun_apply` (for each excess argument).
...
def replaceEPostHeadBot? (goal : MVarId) (target head : Expr) (args : Array Expr) :
  ...
  -- Fold `bot_fun_apply` over the excess arguments: `acc : EPost.cons.head ⊥ x₁ … xᵢ = ⊥`.
  let mut acc := p0
  ...
  for x in args.extract 3 args.size do
    ...
    -- `bfa : (⊥ : σ → τ) x = (⊥ : τ)`
    let bfa := mkApp4 (mkConst ``Lean.Order.bot_fun_apply [predLevel uσ, predLevel uτ]) σ τ τCL x
